...
def main():
    ...
    _st, commit = call("POST", "%s/repos/%s/%s/git/commits" % (API, OWNER, REPO),
                       {"message": "%s 多数据图叠加 · 峰位虚线 · 可删自动峰 · 修复高级设置超屏" % tag,
                        "tree": tree["sha"],
...
### 本次新增
- **多数据图叠加**：多条光谱压在同一套坐标轴上，**每条数据集一种颜色**，
  不上下错开，比的是“谱型像不像”。前 8 条用标准色板，超过 8 条按黄金角
  旋转色相生成新色（60 条内不重色），右侧图例标出来源文件名。
  横坐标**自动取各条谱波数范围的交集**，某条短一截时右边不再空出一段白。
  可一键隐藏峰位数值，只留虚线与标记 —— 十几条谱叠在一起时清爽得多。
  入口【分析工具 → 多数据图叠加（所选光谱）…】或命令行 `--overlay`。
- **峰位波长虚线**：自动识别的峰与手动补标的峰，都从峰顶画虚线引到横坐标轴，
  波数一眼可读。可用 `--no-peak-dash` 关掉。
- **可以删除自动标注的峰**：右键对准某个峰即可删掉离鼠标最近的那个峰，
  自动峰与手动峰都适用。自动峰记进该文件的“已删除”名单，出图、峰列表与
  所有导出都按删除后的结果计算；点【恢复自动峰】一次全恢复。
  为避免误删，要求点在峰附近（横向约 ±1/40 图宽），空白处右键只提示不动作。
- **显示峰位数值开关**：主界面【图表设置】与叠加图对话框各一处，
  命令行 `--no-peak-labels`。
...
### 画质与稳定性
- 六套自测合计 **121 项全部通过**（对话框布局、叠加图配色与峰位虚线的
  像素级校验、中英界面、中英输出、中英命令行、双语内核）。
- 中英两份说明书（`使用说明.txt` / `User_Guide.txt`）同步更新。
